...
def test_add_cdrs(browser, db_connection_brt, db_connection_hrs):
    ...
    try:
        ...
        # находим баланс двух абонентов до всех операций
        cur.execute(f"SELECT balance FROM subscribers WHERE msisdn = '{calls_parameters[8][1]}' LIMIT 1;")
        for row in cur.fetchall():
            start_balance_A1 = float(row[0])
        cur.execute(f"SELECT balance FROM subscribers WHERE msisdn = '{calls_parameters[8][2]}' LIMIT 1;")
        for row in cur.fetchall():
            start_balance_A2 = float(row[0])
        # нужно определить id абонентов для последующих запросов к бд HRS
        cur.execute(f"SELECT id FROM subscribers WHERE msisdn='{calls_parameters[8][1]}' LIMIT 1;")  # Получаем список баз данных, исключая шаблоны
        for row in cur.fetchall():
            id_subscriber_A1 = int(row[0])
        cur.execute(f"SELECT id FROM subscribers WHERE msisdn='{calls_parameters[8][2]}' LIMIT 1;")  # Получаем список баз данных, исключая шаблоны
        for row in cur.fetchall():
            id_subscriber_A2 = int(row[0])
        # подключаемся к БД HRS
        curs = db_connection_hrs.cursor()
        curs.execute(f"""SELECT used_amount FROM subscriber_package_usage WHERE subscriber_id = {id_subscriber_A1} 
                     AND is_deleted=False;""")  # Получаем список баз данных, исключая шаблоны
        for row in curs.fetchall():
            start_minutes_A1 = int(row[0])
        # находим количество минут каждого из абонентов до всех операций
        curs.execute(f"""SELECT used_amount FROM subscriber_package_usage WHERE subscriber_id = {id_subscriber_A2} 
                     AND is_deleted=False;""")  # Получаем список баз данных, исключая шаблоны
        for row in curs.fetchall():
            start_minutes_A2 = int(row[0])
    ...
    try:
        ...
        cur.execute(f"SELECT balance FROM subscribers WHERE msisdn = '{calls_parameters[8][2]}' LIMIT 1;")
        for row in cur.fetchall():
            end_balance_A2 = float(row[0])
        curs.execute(f"""SELECT used_amount FROM subscriber_package_usage WHERE subscriber_id = {id_subscriber_A1} 
                     AND is_deleted=False;""") 
        for row in curs.fetchall():
            end_minutes_A1 = int(row[0])
        # находим количество минут каждого из абонентов после всех операций
        curs.execute(f"""SELECT used_amount FROM subscriber_package_usage WHERE subscriber_id = {id_subscriber_A2} 
                     AND is_deleted=False;""")
        for row in curs.fetchall():
            end_minutes_A2 = int(row[0])
    except psycopg2.Error as e:
        pytest.fail(f"Ошибка при подключении к PostgreSQL: {e}")
    except Exception as e:
        pytest.fail(f"Произошла ошибка: {e}")
