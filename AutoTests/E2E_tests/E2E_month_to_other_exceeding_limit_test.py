...
def test_add_cdrs(browser, db_connection_brt, db_connection_hrs):
    filepath = 'test_e2e/month_to_other_exceeding_limit.txt'
    ...
    try:
        ...
        cur.execute(f"SELECT balance FROM subscribers WHERE msisdn = '{calls_parameters[9][1]}' LIMIT 1;")
        for row in cur.fetchall():
            start_balance_A1 = float(row[0])
        # нужно определить id абонентов для последующих запросов к бд HRS
        cur.execute(f"SELECT id FROM subscribers WHERE msisdn='{calls_parameters[9][1]}' LIMIT 1;")  # Получаем список баз данных, исключая шаблоны
        for row in cur.fetchall():
    ...
    assert start_balance_A1 - end_balance_A1 == 125
